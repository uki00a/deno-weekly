...
上記の場合、`deno task check:all`を実行すると、まず`check:fmt`と`check:deno-json`タスクが並列で実行され、それらが成功した場合のみ`check:all`の`command`が実行されます。
...
`deno task`コマンドに`--filter`オプションが追加されています。このオプションが指定されると、ワークスペース内において、名前がこのオプションで指定されたパターンにマッチするパッケージで定義されたタスクを実行できるようです:
...
指定された引数を[`deno_task_shell`](https://github.com/denoland/deno_task_shell)を使って評価してくれます:
...
`deno.json`で`"unstable": ["fmt-sql"]`を指定する または deno fmtで`--unstable-sql`を指定することで、`.sql`ファイルのフォーマットが有効化されます。
