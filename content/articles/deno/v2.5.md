...
[Deno v2.4]({{< ref "articles/deno/v2.4.md" >}})で非推奨化されていた`--unstable-node-globals`が再導入されています。今回のリリースでは、`--unstable-node-globals`を指定した際に`setTimeout`や`setInterval`を`node:timers`のものに置き換えられるよう機能が導入されています  ([#29993](https://github.com/denoland/deno/pull/29993))
