...
Denoはカレントディレクトリまたはその上位のいずれかのディレクトリに`deno.json(c)`というファイルが存在する場合、それを自動で読み込みます。
...
## (**破壊的変更**) `Worker`内でデフォルトでDeno名前空間を参照できるように変更
...
- HTTPサーバ(`Deno.serveHTTP`)が`ReadableStream`の圧縮がサポートされました。
