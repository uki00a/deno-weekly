...
categories:
  - release
date: 2025-12-14
description: Deno v2.6がリリース (`deno x`コマンドが追加, `deno approve-scripts`コマンドが追加, `deno audit` - `socket.dev`との統合がサポート, `--ignore-read`/`--ignore-env`オプションの追加, `deno publish` - `"publish": false`がサポート, `deno init` -- `--empty`オプションが追加, `deno init --npm` - `--yes`オプションが追加, `deno install -g`の挙動が変更, `--lockfile-only`オプションが追加, `deno lint` - プラグインでファイルと環境変数の読み込みがサポート, `deno bundle` - Androidでの利用がサポート, `Deno.bundle()`の`Worker`での利用がサポート, `deno lsp` - `source.organizeImports`におけるバグ修正, `deno check` - typescript-go連携におけるバグ修正, Source Phase Importsがサポート, `BroadcastChannel`が安定化, Streamの移譲がサポート, Float16Arrayがサポート, Deno本体に`@types/node`がバンドル, `--require`オプションが実装, Subpath importsで`#/`から始まるマッピングがサポート, Windows向けのshimの生成が改善, `node:sqlit` - `DatabaseSync`に`aggregate`や`function`メソッドが追加, `node:child_process` - Advanced serializationがサポート, ソースマップサポートの改善, など)
---
