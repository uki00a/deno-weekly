...
[Deno v1.25.0]({{< ref "/articles/deno/v1.25.md" >}})でnpmパッケージのサポートが入ったためか、Node.js互換モード(`--compat`)が削除するPRが作成されています。
...
このオプションを指定することにより、`npm:`形式での`import`を無効化するできるようにすることが想定されているようです。
...
ReactコンポーネントをSSRした場合などに生成される巨大なHTMLを高速でエスケープする目的で、`Deno.escapeHtml()`というAPIの追加が検討されているようです。
...
Redisから返却されたnull値を表す型である[BulkNil](https://deno.land/x/redis@v0.27.0/mod.ts?s=BulkNil)型が`undefined`から`null`型へ変更されています。(**破壊的変更**)
