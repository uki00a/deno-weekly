...
`Deno.symlink`などのAPIが`--allow-read`パーミッションを要求するように変更されました。
...
`localStorage`で`--location`フラグが不要になりました。
...
詳しくは[こちら](https://github.com/whatwg/dom/commit/4ca187f9ccd4b1548d87293307ee58430ef4908b)
...
- (**破壊的変更**) `ReadableStream#getIterator`が削除されました。
  - `ReadableStreamはAsyncIterable`を実装しているので、今後はそちらを使う必要があります。
...
- `deno fmt --watch` or `deno lint --watch`で引数が与えられなかった際に、うまく動かない問題が修正されました
...
`serveListener`関数が追加されました。使用法はv0.113.0までのserve関数と同様です。
...
[Denoでサーバーを建てる方法 2021年11月版](https://zenn.dev/kawarimidoll/articles/8031c2618fedca)の記事で`serve`などについて解説されているので、よろしければ参照ください。
...
- `readline`モジュールが追加されました (下記関数がサポート)
  - `clearLine`
  - `clearScreenDown`
  - `cursorTo`
  - `moveCursor`
...
ファイル配信時に、`ReadableStream`を使用してストリーミングされるように修正されました。
