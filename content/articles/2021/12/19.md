...
new TextDecoder().decode(new Deno.UnsafePointerView(p).getArrayBu
ffer(5)); // => "Hello"
```
...
背景としては、後述するEdgeDBドライバをDenoで実装する上で、ALPNサポートが必要になるためサポートされたようです。
...
Node.js互換モード(`--compat`)の互換性向上に向けて、様々な機能が実装されていｍさう。
...
Denoの[EdgeDB](https://github.com/edgedb/edgedb)
