...
date: 2025-10-26
description: >
  Denoの直近のアップデート (typescript-goサポートがマージ, `deno.json`での`minimumDependencyAge`の指定と特定のパッケージの除外がサポート, `deno audit`コマンドがマージ, など), Deno Deployのアップデート (Databases - データベースマイグレーションのサポート), freshのリリース (@fresh/init@2.2.1 - `@/`形式のパスエイリアスが設定されるよう改善, @fresh/core@2.1.3 - HEADの取り扱いの改善, など)
---
