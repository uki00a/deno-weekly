...
date: 2025-10-19
description: >
  Denoにおけるサプライチェーン攻撃関連の対応について (`--minimum-dependency-age`と`deno.json`への`minimumDependencyAge`の追加について), Hono v4.10.0 (`hono/client` - Middlewareに関する型推論の改善, `hono/request` - `cloneRawRequest`が追加, `hono/ssg` - `defaultPlugin`が追加)
---
