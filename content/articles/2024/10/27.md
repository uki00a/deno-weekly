...
date: 2024-10-27
description: >
  Denoのv2.0.3 (Interactive Inlay Hints for Typesのサポート, deno compileでのNode-APIのサポート, など), @deno/nextjs-start (Deno Deployでnext startを動かすためのパッケージ), @std/async@1.0.7 (throttleが実装), @std/collections@1.0.9 (slidingWindowsでIterableがサポート), @std/expect@1.0.6 (expect.objectContainingが実装)
---
