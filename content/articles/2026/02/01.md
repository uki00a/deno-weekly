...
date: 2026-02-01
description: >
  Deno v2.6.7 (`@jsr`スコープの取り扱いの変更, `Deno.cron()` - `DENO_UNSTABLE_CRON_SOCK`による外部サーバーへのタスク管理の移譲がサポート, `node:test` - 各種`mock.*`APIの実装, `node:sqlite` - `DatabaseSync`で未実装であった各種オプションのサポート, など), deno_stdのリリース (@std/http@1.0.24 - 実験的APIとして`ServerSentEventParseStream`/`FormDataDecoderStream`/`FormDataEncoderStream`が追加, `@std/tar@0.1.10` - `TarStream`でシンボリックリンクがサポート, など)
---
